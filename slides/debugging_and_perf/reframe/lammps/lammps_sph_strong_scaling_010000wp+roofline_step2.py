...
class LAMMPSPerfHackathon(RunOnlyRegressionTest):
    def __init__(self, name, **kwargs):
        ...
        self.pre_run += ['XX=/apps/common/UES/intel/2019 && AA=$XX/advisor_2019 && ' \
                         'source $AA/advixe-vars.sh && advixe-cl -V ;']
...
class LAMMPSStrongScaling(LAMMPSPerfHackathon):
    def __init__(self, num_tasks, **kwargs):
        super().__init__('LAMMPS_%s_010000wp_strong+roofline_step2' % (str(num_tasks)), **kwargs)

        self.executable = 'advixe-cl -collect tripcounts -flop -trace-mpi -project-dir=$SCRATCH/roofline/lammps_roofline_%s -data-limit=0 --no-auto-finalize lmp_mpi' \
                % (str(num_tasks))
