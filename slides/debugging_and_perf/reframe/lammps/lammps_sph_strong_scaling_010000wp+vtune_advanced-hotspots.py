...
class LAMMPSPerfHackathon(RunOnlyRegressionTest):
    def __init__(self, name, **kwargs):
        ...
        self.pre_run += ['rm -fr $SCRATCH/lammps/vtune/hotspots2 ;']
    ...
    def setup(self, system, environ, **job_opts):
        ...
        # Access to /proc/kallsyms file is limited
        self.job.launcher.options += ['-C perf']


class LAMMPSStrongScaling(LAMMPSPerfHackathon):
    def __init__(self, num_tasks, **kwargs):
        super().__init__('LAMMPS_%s_010000wp_strong+vtune_hotspots2' % (str(num_tasks)), **kwargs)

        self.executable = 'amplxe-cl -collect advanced-hotspots -trace-mpi -r $SCRATCH/lammps/vtune/hotspots2/%s -data-limit=0 lmp_mpi' \
                % (str(num_tasks))
