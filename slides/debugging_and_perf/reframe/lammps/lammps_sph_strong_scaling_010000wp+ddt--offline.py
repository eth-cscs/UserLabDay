...
class LAMMPSPerfHackathon(RunOnlyRegressionTest):
    def __init__(self, name, **kwargs):
        ...
        self.modules = ['LAMMPS/16Jul2018-CrayGNU-18.07-cuda-9.1', 'ddt/18.1.3-Suse-12']
        ...
        #self.post_run += ['cat ddt.txt']
        #self.post_run += ['cat ddt.html']
        self.variables = {
    ...
    def setup(self, system, environ, **job_opts):
        ...
        self.job.launcher = LauncherWrapper(self.job.launcher, 'ddt --offline --trace-at pair_sph_taitwater.cpp:194,0:2:20,i,j,nlocal,newton_pair')
        self.job.launcher.options = ['--unbuffered']


class LAMMPSStrongScaling(LAMMPSPerfHackathon):
    def __init__(self, num_tasks, **kwargs):
        super().__init__('LAMMPS_%s_100000wp_strong+ddt_offline' % (str(num_tasks)), **kwargs)
