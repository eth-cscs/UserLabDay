...
class LAMMPSStrongScaling(LAMMPSPerfHackathon):
    def __init__(self, num_tasks, **kwargs):
        super().__init__('LAMMPS_%s_010000wp_strong+roofline_step1' % (str(num_tasks)), **kwargs)

        self.executable = 'advixe-cl -collect survey -trace-mpi -project-dir=$SCRATCH/roofline/lammps_roofline_%s -data-limit=0 lmp_mpi' \
                % (str(num_tasks))
