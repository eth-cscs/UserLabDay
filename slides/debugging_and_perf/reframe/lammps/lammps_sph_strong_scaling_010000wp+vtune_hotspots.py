...
class LAMMPSStrongScaling(LAMMPSPerfHackathon):
    def __init__(self, num_tasks, **kwargs):
        super().__init__('LAMMPS_%s_010000wp_strong+vtune_hotspots' % (str(num_tasks)), **kwargs)

        self.executable = 'amplxe-cl -collect hotspots -trace-mpi -r $SCRATCH/lammps/vtune/hotspots/%s -data-limit=0 lmp_mpi' \
                % (str(num_tasks))
